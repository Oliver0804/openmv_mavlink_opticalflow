...
while True:
    ...
    if displacement.response() > 0.7:
        draw_color = (0, 255, 0)  # 绿色
    elif displacement.response() < 0.7 and displacement.response() > 0.5:
        draw_color = (242, 133, 0)  # 橙色
    else:
        draw_color = (255, 0, 0)  # 红色

    #
    radius = min(int((abs(sub_pixel_x) + abs(sub_pixel_y)) / 2), 20)  # 计算半径，并限制最大值
    if radius < 5:
        #
        img.draw_circle(32, 32, radius, color=(0, 255, 0), thickness=2)
    else:
        #
        img.draw_arrow(32, 32, int(sub_pixel_x/2) + 32, int(sub_pixel_y/2) + 32, draw_color, thickness=2)
